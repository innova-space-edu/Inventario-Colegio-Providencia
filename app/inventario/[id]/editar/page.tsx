...
export default async function EditAssetPage({
  ...
  const [{ data: rawAsset }, { data: familyData }, { data: statusData }, { data: locationData }] = await Promise.all([
    supabase.from("assets").select("*,family:asset_families(code,name)").eq("id", id).maybeSingle(),
  ...
  const asset = rawAsset as typeof rawAsset & { family: Relation<{ code: string; name: string }> };
  const family = relationOne(asset.family);
  let details: Partial<AssetFormInitial> = {};
  ...
  return (
    <AppShell>
      <header className="topbar">
        <div><h1>Editar activo</h1><p>{asset.inventory_code || asset.name || "Registro de inventario"} · {family?.name || "Sin familia"}</p></div>
        <Link className="button button-ghost" href={`/inventario/${id}`}>Cancelar</Link>
      </header>
      <AssetForm action={updateAsset} error={error} families={families} initial={initial} locations={locations} mode="edit" statuses={statuses} />
    </AppShell>
  );
}
