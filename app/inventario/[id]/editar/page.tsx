...
export default async function EditAssetPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const [{ data: rawAsset }, { data: familyData }, { data: statusData }, { data: locationData }] = await Promise.all([
    ...
    supabase.from("locations").select("id,name,area,category,display_order,selectable").eq("active", true).order("category").order("display_order").order("name"),
  ]);
  ...
  if (family?.code === "computer") {
    ...
    const { data } = await supabase.from("projector_details").select("lumens,hdmi,vga").eq("asset_id", id).maybeSingle(); details = data ?? {};
  } else if (family?.code === "television") {
    const { data } = await supabase.from("television_details").select("size").eq("asset_id", id).maybeSingle(); details = data ?? {};
  }
