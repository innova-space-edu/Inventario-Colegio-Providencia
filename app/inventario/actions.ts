...
function detailPayload(formData: FormData) {
  return {
    memory: text(formData, "memory"), storage: text(formData, "storage"), screen: text(formData, "screen"),
    keyboard: text(formData, "keyboard"), battery: text(formData, "battery"), charger: text(formData, "charger"),
    lumens: text(formData, "lumens"), hdmi: text(formData, "hdmi"), vga: text(formData, "vga"), size: text(formData, "size"),
  };
}
...
export async function createAsset(formData: FormData) {
  ...
  const { data: assetId, error } = await supabase.rpc("create_asset_atomic", {
    p_asset: { ...commonPayload(formData), family_id: familyId },
    p_details: detailPayload(formData),
  });
  ...
  redirect(`/inventario/${assetId}?created=1`);
}
...
export async function updateAsset(formData: FormData) {
  ...
  const { error } = await supabase.rpc("update_asset_atomic", {
    p_asset_id: assetId,
    p_asset: commonPayload(formData),
    p_details: detailPayload(formData),
  });
