...
function quantity(formData: FormData) {
  const value = Number(formData.get("quantity") ?? 1);
  return Number.isInteger(value) && value > 0 ? value : 1;
}

async function saveFamilyDetails(
  ...
  if (familyCode === "computer") {
    const { error } = await supabase.from("computer_details").upsert({
      asset_id: assetId,
  ...
  if (familyCode === "projector") {
    const { error } = await supabase.from("projector_details").upsert({
      asset_id: assetId,
  ...
  if (familyCode === "television") {
    const { error } = await supabase.from("television_details").upsert({
      asset_id: assetId,
...
export async function createAsset(formData: FormData) {
  ...
  await supabase.from("asset_history").insert({
    asset_id: asset.id,
    event_type: "created",
    description: "Activo creado desde la aplicación web.",
    after_data: payload,
    actor_id: profile.id,
  });
  ...
  redirect(`/inventario/${asset.id}?created=1`);
}
...
export async function updateAsset(formData: FormData) {
  ...
  await supabase.from("asset_history").insert({
    asset_id: assetId,
    event_type: "updated",
    description: "Activo actualizado desde la aplicación web.",
    before_data: before,
    after_data: { ...before, ...payload },
    actor_id: profile.id,
  });
  ...
  redirect(`/inventario/${assetId}?updated=1`);
}
...
export async function disposeAsset(formData: FormData) {
  ...
  const observations = text(formData, "disposal_observations");
  const approvedBy = text(formData, "approved_by");

  const [{ data: asset }, { data: disposedStatus }] = await Promise.all([
    supabase.from("assets").select("*").eq("id", assetId).single(),
    supabase.from("asset_statuses").select("id").eq("code", "disposed").single(),
  ]);
  ...
  const { error: updateError } = await supabase.from("assets").update({
    is_disposed: true,
    status_id: disposedStatus.id,
    updated_by: profile.id,
  }).eq("id", assetId);
  if (updateError) redirect(`/inventario/${assetId}?error=dispose`);

  const { error: disposalError } = await supabase.from("asset_disposals").insert({
    asset_id: assetId,
    reason,
    observations,
  ...
  if (disposalError) redirect(`/inventario/${assetId}?error=dispose_record`);

  await supabase.from("asset_history").insert({
    asset_id: assetId,
    event_type: "disposed",
    description: reason || "Activo dado de baja.",
    before_data: asset,
    after_data: { ...asset, is_disposed: true, status_id: disposedStatus.id },
    actor_id: profile.id,
  });
