...
export async function GET(request: NextRequest) {
  ...
  if (!profile?.active) return new Response("Sin autorización", { status: 403 });
  const { data: canExport, error: permissionError } = await supabase.rpc("has_permission", { p_permission: "reports.export" });
  if (permissionError || canExport !== true) return new Response("Sin autorización", { status: 403 });
  ...
  const rows: ExportRow[] = []; const chunkSize = 1000; let offset = 0;
  while (true) {
    ...
    if (scope === "active") query = query.eq("is_disposed", false); if (scope === "disposed") query = query.eq("is_disposed", true); if (family) query = query.eq("family_id", family.id); if (status) query = query.eq("status_id", status.id); if (location) query = query.eq("location_id", location.id);
    const { data, error } = await query; if (error) return new Response("No fue posible generar la exportación", { status: 500 });
    const chunk = (data ?? []) as unknown as ExportRow[]; rows.push(...chunk); if (chunk.length < chunkSize) break; offset += chunkSize;
  }
  ...
  const date = new Date().toISOString().slice(0, 10); const parts = ["inventario-colegio-providencia", scope, filenamePart(familyCode), filenamePart(statusCode), filenamePart(locationId), date].filter(Boolean); const filename = `${parts.join("-")}.csv`;
