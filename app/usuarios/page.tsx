...
export default async function UsersPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return (
    <AppShell>
      ...
      {errorCode ? <div className="error-box">No se pudo completar la operación de usuarios ({errorCode}). Revisa los datos y vuelve a intentarlo.</div> : null}
      ...
      <section className="panel panel-flush">
        ...
        <form action={createManagedUser} className="form-grid">
          ...
          <label className="field"><span>Rol inicial</span><select name="role_id" required><option value="">Selecciona un rol</option>{assignableRoles.map((role) => <option key={role.id} value={role.id}>{role.name}</option>)}</select></label>
          <label className="field field-wide"><span>Contraseña temporal (opcional, mínimo 10 caracteres)</span><input name="temporary_password" type="password" autoComplete="new-password" placeholder="Déjala vacía para enviar una invitación" /></label>
