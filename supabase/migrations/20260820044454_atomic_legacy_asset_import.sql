-- Importación atómica de una fila legado ya preservada en legacy_imports.
-- La función solo puede ejecutarse con service_role y evita activos parciales.
...
begin
  ...
  if v_asset_id is null then
    ...
    if v_inventory_code is not null and exists (
      select 1
      from public.assets a
      where a.inventory_code is not null
        and upper(a.inventory_code) = upper(v_inventory_code)
    ) then
    ...
    insert into public.assets(
      ...
      v_status_id,
      p_location_id,
      nullif(btrim(p_asset->>'name'), ''),
      nullif(btrim(p_asset->>'asset_type'), ''),
      nullif(btrim(p_asset->>'brand'), ''),
      nullif(btrim(p_asset->>'model'), ''),
      nullif(btrim(p_asset->>'serial_number'), ''),
      v_quantity,
      nullif(btrim(p_asset->>'area'), ''),
      nullif(btrim(p_asset->>'observations'), ''),
      false,
      v_source_table,
      v_source_id,
      v_payload,
  ...
  if p_family_code = 'computer' then
    insert into public.computer_details(
      ...
      v_asset_id,
      nullif(btrim(p_details->>'memory'), ''),
      nullif(btrim(p_details->>'storage'), ''),
      nullif(btrim(p_details->>'screen'), ''),
      nullif(btrim(p_details->>'keyboard'), ''),
      nullif(btrim(p_details->>'battery'), ''),
      nullif(btrim(p_details->>'charger'), ''),
      v_payload
    )
    ...
    values (
      v_asset_id,
      nullif(btrim(p_details->>'lumens'), ''),
      nullif(btrim(p_details->>'hdmi'), ''),
      nullif(btrim(p_details->>'vga'), ''),
      v_payload
    )
    on conflict (asset_id) do update set
  ...
  if v_created then
    ...
    insert into public.asset_history(
      asset_id, event_type, description, after_data, actor_id
    ) values (
      v_asset_id,
      'legacy_import',
...
comment on function public.import_legacy_asset_atomic(bigint, text, uuid, jsonb, jsonb) is
  'Importa o repara una fila legado y sus detalles especializados en una sola transacción. Solo service_role.';
